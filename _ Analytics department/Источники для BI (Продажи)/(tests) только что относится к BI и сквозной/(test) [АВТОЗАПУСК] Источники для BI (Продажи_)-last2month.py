...
path_SQL_sales = glob.glob(abs_path + r'**/' + r'*test*АВТОЗАПУСК] Источники для BI (Продажи_).sql', recursive=True)[0]
...
df_fon = pd.DataFrame()
for i in df_date_m.iloc[-4:,:].values:
# for i in df_date_m.iloc[-20:,:].values:
