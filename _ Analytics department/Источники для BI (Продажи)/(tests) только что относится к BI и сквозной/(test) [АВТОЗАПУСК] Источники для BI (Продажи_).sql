/* 
 * ИЗМЕНЕННЫЙ фон для BI  (Продажи все позиции без ограничений) 
 * 238 222 01.11.2021 01.12.2021
 * */
...
, wares as  (
    select
        ...
        ,a.in_account_date in_account_date --Время добавления позиции в заказ
...
, services AS  (
    ...
    LEFT JOIN opentech.accounts a ON a.i = os.orderitemid
    LEFT JOIN opentech.users u ON lower(u.login) = lower(a.user_login)
    LEFT JOIN orders.realization_items_order_items_bindings bi ON bi.id_order_item = a.i
    LEFT JOIN orders.realization_items i ON i.id = bi.id_realization_item
    LEFT JOIN orders.realizations r ON r.id = i.id_realization
    ...
    LEFT JOIN public.nds rnds ON rnds.id = i.id_nds
    LEFT JOIN public.nds cnds ON cnds.code = ci.vat
    LEFT JOIN service.types t ON t.id = os.type
    LEFT JOIN service.categories c ON c.id = t.id_category
	JOIN opentech.account_item ai ON ai.account_id = a.id
...
, deliveries AS  (
    ...
    LEFT JOIN logistics.delivery_items di ON di.id_delivery = d.id
    LEFT JOIN opentech.accounts a ON a.i = di.id_order_item AND a.correct_name ILIKE 'Доставка%'
    LEFT JOIN opentech.users u ON lower(u.login) = lower(a.user_login)
    LEFT JOIN orders.realization_items_order_items_bindings bi ON bi.id_order_item = a.i
    LEFT JOIN orders.realization_items i ON i.id = bi.id_realization_item
    LEFT JOIN orders.realizations r ON r.id = i.id_realization
...
, giftcards AS  (
    ...
    LEFT JOIN opentech.accounts a ON a.i = g.id_order_item
    LEFT JOIN opentech.users u ON lower(u.login) = lower(a.user_login)
    LEFT JOIN orders.realization_items_order_items_bindings bi ON bi.id_order_item = a.i
    LEFT JOIN orders.realization_items i ON i.id = bi.id_realization_item
    LEFT JOIN orders.realizations r ON r.id = i.id_realization
...
--    WHERE 
--        d.state = logistics.delivery_state('CLOSED')
...
*/
    ...
    ai.tm_created::date "Дата создания заказа",
    to_char(ai.tm_created, 'HH24:MI') "Время создания заказа",
    
    ai.tm_placed::date "Дата оформления заказа",
    to_char(ai.tm_placed, 'HH24:MI') "Время оформления заказа",
        
    item.in_account_date::date "Дата добавления позиции в заказ",
    to_char(item.in_account_date, 'HH24:MI') "Время добавления позиции в заказ",
/*
...
--    CASE WHEN o.is_full_sold THEN o.date_sold END "Дата полной отгрузки заказа",
    ...
    item.item_state AS "Состояние позиции",
...
*/
    ,item.adel "Позиция удалена"

FROM params _
CROSS JOIN items item
LEFT JOIN store.shipping_states sold_state ON sold_state.id = item.state
--LEFT JOIN orders o ON o.id_order = item.id_order
/* клиент */
LEFT JOIN opentech.customers customer ON customer.id = item.id_customer
/*
...
*/
/* место продажи */
LEFT JOIN opentech.storage storage ON storage.id = item.id_storage
LEFT JOIN public.cities city ON city.id = storage.city_id
...
null, --"Время создания заказа", 
null, --"Дата добавления позиции в заказ",
null, --"Время добавления позиции в заказ",
...
select --i,
    "ID клиента","e2e4 ID","ID оформившего заказ","Ответственный менеджер","ID заказа","ID площадки","Дата создания заказа"
    ,"Время создания заказа","Дата оформления заказа","Время оформления заказа","Дата добавления позиции в заказ","Время добавления позиции в заказ"
    ,"Дата отгрузки товара","Время отгрузки","Тип позиции","Артикул","Наименование позиции","Производитель","Категория","Поставщик"
    ,"Расчетная себестоимость","Себестоимость чистая","Продажная цена","Состояние позиции","Статус списания","ID склада отгрузки","Дата реализации"
    ,"Способ доставки","Транспортная компания","Регион доставки","Город доставки","Адрес доставки","Сумма НДС","Фирма-поставщик"
    ,"Фирма-грузоотправитель","сборка/комплект","Кол-во сборок","Позиция удалена"
from UNION_ALL_END UE


/*-- ДУБЛИ ПО i ИДКТ НО ПОЛУЧАЕТСЯ ЧТО ДОСТАВКА И ТОВАР i ОДИНАКОВОЕ...!!!!!!*/
...
--/*ПРОВЕРКА НА ДУБЛИ ПО i*/
--,CHECK_FOR_DUPLICATES as ( 
--select count(UE.i), UE.i from UNION_ALL_END UE
--group by
--	UE.i --UE.i --2-39 --12 788
--HAVING
--	count(UE.i) > 1
--)

--select * from UNION_ALL_END UE
--where UE.i in (select i from CHECK_FOR_DUPLICATES)	
